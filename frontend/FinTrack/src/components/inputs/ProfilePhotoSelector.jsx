...
const ProfilePhotoSelector = ({ image, setImage }) => {
  ...
  const handleImageChange = (event) => {
    const file = event.target.files[0]; // when a user selects a file to choose it as display browser gives it in event.target.files and we choose the first file in it therefore event.target.files[0];
    if (file) {
      //update the image state
      setImage(file);

      //generate preview URL from the file
      const preview = URL.createObjectURL(file); // You create a temporary URL using URL.createObjectURL(file) so you can show an immediate preview (instead of waiting to upload to a server).
